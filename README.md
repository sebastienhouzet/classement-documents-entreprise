...
française — juridique, contrats, RH, comptabilité, fiscalité, banque, assurances, administratif — avec, dans
chaque dossier, un fichier `index.md` qui explique quoi y mettre, comment le classer et combien de temps le garder.
...
2. **Lire trois fichiers** (10 minutes) :
   - [`index.md`](index.md) — le guide de classement : les règles, la routine, qui a accès à quoi ;
   - [`00 - REFERENTIEL/Convention-de-nommage.md`](00%20-%20REFERENTIEL/Convention-de-nommage.md) — comment nommer fichiers et dossiers ;
   - [`00 - REFERENTIEL/Durees-de-conservation.md`](00%20-%20REFERENTIEL/Durees-de-conservation.md) — le tableau des durées légales et recommandées.
3. **Remplir les registres CSV** avec l'existant (contrats, assurances, matériel) : ce sont eux qui donnent la
   vue d'ensemble et les dates d'échéance.
4. **Ranger au fil de l'eau** : à chaque document, ouvrir l'`index.md` du dossier concerné en cas de doute.
   Les sous-dossiers par client, par salarié ou par année se créent quand le besoin apparaît, pas à l'avance.
...
├── index.md                           ← guide de classement (règles, routine, accès)
├── 00 - REFERENTIEL/                  ← convention de nommage, durées de conservation, kit administratif
...
├── 05 - BANQUE & FINANCEMENT/         ← comptes, emprunts, aides et subventions, investisseurs, moyens de paiement, garanties
...
└── 08 - ARCHIVES/                     ← dossiers clos en attente de destruction
```

Chaque domaine est découpé en sous-dossiers numérotés (`01.1`, `01.2`…), soit **64 dossiers** au total.
...
| Logique | Où | Exemple |
|---|---|---|
| **Par tiers** | Clients, fournisseurs, salariés, contrats d'assurance, comptes bancaires | `02.1 - Clients/SAS Dupont/` |
| **Par année, puis par mois** | Factures, relevés, paie, notes de frais, courrier | `04.3 - Factures fournisseurs/2026/2026-03/` |
| **Par opération ou affaire** | AG, modifications statutaires, litiges, sinistres, prêts, levées de fonds | `01.9 - Contentieux/2026 - SAS Dupont - Impayé/` |
...
Six fichiers CSV vides (séparateur `;`, encodage UTF-8, ouvrables dans Excel, Numbers ou LibreOffice) servent de
...
| `Registre-des-archives.csv` | `08 - ARCHIVES/` | Dossiers clos et **dates de destruction prévues** |
...
2026-03-15_Contrat_Dupont_Refonte-site_signe.pdf
2026-04-02_Facture_Hosting_Hebergement-mars_35.88.pdf
...
mois `AAAA-MM`. Le détail est dans [`Convention-de-nommage.md`](00%20-%20REFERENTIEL/Convention-de-nommage.md).
...
[`Durees-de-conservation.md`](00%20-%20REFERENTIEL/Durees-de-conservation.md). Quelques repères :
...
- **Supprimer** ou laisser vides les dossiers qui ne vous concernent pas (`03.10` sans CSE, `07.4` sans
  véhicule, `05.4` sans investisseurs…). Leur `index.md` explique à partir de quand ils deviennent nécessaires.
...
- **Ajouter** des sous-dossiers de niveau 3 librement (par tiers, par année, par opération) selon les règles de
  chaque `index.md`.
- **Restreindre les accès** : `03 - RESSOURCES HUMAINES` contient des données personnelles et de santé, et doit
  avoir ses propres droits si le stockage est partagé. La grille d'accès proposée est dans [`index.md`](index.md).
- **Ne jamais versionner de documents réels** dans un dépôt Git public : si vous forkez ce dépôt pour l'utiliser,
  ajoutez un `.gitignore` qui n'autorise que les `*.md` et les `*.csv` de gabarit, ou travaillez hors Git.
...
Ce gabarit est une aide à l'organisation, pas un conseil juridique. Les durées de conservation indiquées
correspondent aux textes en vigueur à la date de rédaction (septembre 2026) et sont données à titre indicatif ;
vérifiez-les pour votre situation, notamment auprès de votre expert-comptable ou de votre avocat, et consultez la
fiche officielle « Durée de conservation des documents d'une entreprise » sur service-public.fr, mise à jour
régulièrement.
